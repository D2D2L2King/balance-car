 ...
 * min(int a, int b)
 */
#define MPU6050										//定义用到的芯片型号

//2100行设置重力方向，方向矩阵需要手动设置

#if defined STM32F103xB								//你自己使用的MCU芯片(在 stm32f4xx.h 中被定义)

#include "MPU6050.h"						//mpu6050的底层驱动, 符合DMP下面的 i2c_write, i2c_read 两函数
#include "stm32f1xx.h"								//芯片型号的头文件
#include "i2c.h"									//HAL库用到的i2c接口
#include "stm32f1xx_hal.h"							//HAL库没有为专门延时单独写个库, Hal_Delay 在stm32l0xx_hal.h中
...
//#define i2c_write  		I2C_Write					//i2c写
//#define i2c_read   		I2C_Read					//i2c读
#define delay_ms   		HAL_Delay					//延时
#define get_ms     		myget_ms					//时间戳(不需要)
#define log_i     		printf						//串口发送
#define log_e     		printf						//串口发送
/* labs is already defined by TI's toolchain. */
/* fabs is for doubles. fabsf is for floats. */
#define fabs       		fabsf						//C语言编译文件, 在 math.h 头文件中
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    accel[2] = (long)(((long long)accel[2]<<16) / test.accel_sens /
        packet_count);
    /* Don't remove gravity! */
    //0是x轴，1是y轴，2是z轴，DMP默认程序写的是accel[2]，
    //所以只能通过将芯片水平放置让z轴和G重合才能让 mpu_run_self_test 函数自检成功
